...
```text
STM32 SC_TYPE_IMU_TELEMETRY (0x27)
...
- 潜在影响：仅改变 BMI 状态显示来源；不改变 BLE、SCBP payload、采样频率、LSM/BMI 数据接收或校准状态机。
...
- 不修改 `SC_TYPE_IMU_TELEMETRY=0x27`、BLE FFE2 或 payload 格式。
...
- 潜在影响：仅影响控制页 BMI availability 文本；不改变 SCBP type/payload、BLE characteristic、采样频率、校准状态机或传感器数据接收。
