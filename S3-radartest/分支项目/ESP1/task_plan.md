...
# Task Plan: P1 Sensor Ingress and P2 C5 Voice Resource Arbitration

## Goal

Complete the 2026-07-13 P1 and P2 implementation plan in order: make the S3
`/local/v1/sensor` HTTP ingress fail fast under runtime contention, then add a
mirrored C51/C52 voice resource arbiter that owns quiesce, lease, release, and
recovery boundaries.

## Scope and guardrails

- Modify only `ESPS3`, `ESPC51`, `ESPC52`, and necessary audit/verification docs.
- Do not modify or start `ESP-server`, its public assets, database, managed components,
  or archive content.
- Preserve existing uncommitted changes and C51/C52 shared-code parity.
- Do not flash, monitor, erase flash, or fullclean.
- Keep BME/CSI payloads and algorithms, Server APIs, Dashboard behavior, and raw CSI
  upload boundaries unchanged.

## Phases

- [x] Phase 1: Read the user plan, project context, current audit/design documents, and
  both Git baselines.
- [x] Phase 2: P1 trace and minimal implementation for bounded sensor ingress waits,
  diagnostics, and timeout-to-503 mapping.
- [x] Phase 3: Build and static-verify P1 before beginning P2.
- [x] Phase 4: Implement the C51/C52 shared `c5_resource_manager` and integrate bounded
  BME, CSI, background, Mic, upload, response, and speaker lifecycle arbitration.
- [x] Phase 5: Build all firmware targets, run parity and negative scans, and write the
  final execution report with hardware-validation requirements.

## Baseline decisions

| Decision | Reason |
|---|---|
| Treat the attached P1/P2 plan as the approved implementation design | It specifies ordering, state semantics, timeouts, rollback behavior, and acceptance criteria. |
| Keep prior streaming/memory-stability changes intact | The worktree is already dirty; reverting or overwriting them would violate scope. |
| Supersede only the conflicting startup-resident speaker assumption | The current P2 requirement explicitly keeps speaker I2S/DMA/session resources out of standby. |
| Gate P2 on a completed P1 build | The user explicitly requires P1 completion and verification first. |

## P1 completion evidence

- `/local/v1/sensor` remains body receive, validation, resource-session snapshot, and
  asynchronous scheduler ingress only; sensor aggregation, cache, network worker, and
  Server HTTP remain downstream of the handler.
- The sensor-specific path uses one `LOCAL_HTTP_SENSOR_INGRESS_ADMISSION_TIMEOUT_MS = 100U`
  deadline across resource-session snapshot and event-bus admission; the second stage only
  receives the remaining budget.
- A timed lock failure returns `ESP_ERR_TIMEOUT`, which the existing local error mapping
  sends as `503 Service Unavailable` with the existing local timeout error code.
- Sensor ingress diagnostics are success-rate-limited and failure-complete; they record
  receive/enqueue timing, queue depth where safely available, and internal/DMA heap
  values without taking a second blocking bus lock.
- `idf.py build` completed successfully in `ESPS3` after the P1 changes. No hardware,
  flash, monitor, or Server process was used.

## P2 completion evidence

- C51/C52 now share a byte-identical `c5_resource_manager` with `STANDBY`,
  `QUIESCING`, `VOICE_EXCLUSIVE`, `RELEASING`, and `ERROR` state transitions, a single
  generation lease, bounded HTTP/BME/CSI/worker ACK waits, and reverse rollback.
- Normal HTTP admission now counts in-flight work and blocks health/register reconnection
  traffic during an active lease. A reconnect blocked by voice is deferred without being
  counted as a link failure.
- Audio cleanup is fail-closed: receiver shutdown and speaker-session release must finish
  before CSI, BME, workers, and normal HTTP resume. A bounded failure retains the lease and
  schedules a generation-checked retry instead of restarting Mic or resuming background work.
- Terminal voice events use a reliable priority slot plus task notification and lease
  generation checks; stale callbacks and a full ordinary event queue cannot release a newer
  voice turn. The response worker freezes and passes its originating lease to terminal
  callbacks, and link-loss aborts are ignored in standby.
- Speaker/I2S/session allocation remains lazy. Phase logs cover upload allocation/free,
  wake/response playback, and post-deinit release. C51/C52 builds, parity comparison,
  `git diff --check`, and targeted scope scans completed successfully.
