...
LSM303/BMI323 acquisition
    ...
    -> SRP telemetry and schema-2 DualAHRS attitude
```
...
- `IMU_CAL_STATUS(0x12)`: 11-byte lifecycle status.
- `IMU_TELEMETRY(0x10)`: one 30-byte frame per sensor. Sensor ID 1 is
  LSM303 and sensor ID 2 is BMI323.
- `ATTITUDE(0x11)`: one 80-byte schema-2 DualAHRS payload only.
- `CAL_EVENT(0x01, STATIC_DONE=1)`: emitted after the admitted static window.
