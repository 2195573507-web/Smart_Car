...
Implement the approved minimal extension across the existing SRPv4,
...
The existing SRPv4 envelope remains unchanged.

## Message Contracts

| Direction | Message | ID/type | Payload |
| --- | --- | --- | --- |
| S3 -> STM | wheel command | SRPv4 `0x02`, ACK_REQUIRED | 4 x float32 LE, 16 B |
| STM -> S3 | wheel status | SRPv4 `0x14`, STREAM_DATA; App `0x16` | 4 x float32 LE, 16 B |
| STM -> S3 | power status | SRPv4 `0x13`, STREAM_DATA; App `0x1C` | float32 LE, 4 B |
...
validates the BLE frame and forwards the payload as an ACK-required SRPv4
command. STM validates length and finite float values, updates targets, and
returns an SRPv4 OK response. The STM MotorBoard task applies one position PID
...
- SRPv4 invalid length or non-finite wheel payload receives
  `SRP_FAST_RESP_INVALID_PARAM`.
- SRPv4 transaction failure is reported to the App through the existing ACK type.
...
Run the Common SRP host suite, a strict Swift package build/test, a CM7
