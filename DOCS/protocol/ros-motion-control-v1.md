...
2. Windows replies `HELLO_ACK`, then sends `LEASE_REQUEST` with a non-zero
   `lease_id`. S3 grants exactly one lease for this TCP session.
3. The lease TTL is 20..220 ms; the S3 default is 220 ms. Each accepted motion
   command refreshes the lease deadline only within that bound. Heartbeats may
   refresh liveness but never create a lease.
4. `MOTION_CMD` requires the current session, current lease, a newer sequence,
   finite values within the configured limits, and `ttl_ms` in 20..220 ms.
5. `STOP` is explicit and accepted only for the current session/lease. It
   clears the latest command and sends a zero-speed SRP command.
6. Lease expiry, malformed/unauthenticated input, sequence disorder, TCP
...
7. After any revoke, recovery requires a new TCP handshake and a new lease;
