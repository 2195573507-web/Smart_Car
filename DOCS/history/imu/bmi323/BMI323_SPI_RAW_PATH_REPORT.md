...
The output uses the existing `bsp_uart_log_write_link_level()` interface. It emits the established SRPv4 `SRP_MSG_ID_LOG (0x30)` frame over USART2, where the existing S3 log bridge forwards it to FFE3 for the App logger.
