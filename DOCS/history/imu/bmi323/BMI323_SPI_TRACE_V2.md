...
The trace uses the existing `bsp_uart_log_write_link_level()` interface, which emits the established SRPv4 `SRP_MSG_ID_LOG (0x30)` frame through USART2 and the existing S3 FFE3/App logger path. No protocol, message ID, or BLE code changes are made.
