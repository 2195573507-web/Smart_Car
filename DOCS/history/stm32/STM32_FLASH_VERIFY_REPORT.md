...
> Historical snapshot: this report predates the SRPv4 full switch. Its flash
> hashes and startup log observations are retained as evidence only; current
> protocol details are defined by `Common/SRP/` and `DOCS/SRP_v4_Spec.md`.
...
startup path and capture the existing USART2/SRP LOG-to-BLE log stream:
