...
| App protocol | `IOS_APP/SmartCar_Control_MAC/.../Model` | `SmartCarProtocol.Parser.feed` | `encode`, CRC, decoded messages | App frame model |
| App BLE | `IOS_APP/SmartCar_Control_MAC/.../BLE` | `BLEManager` | scan/connect/send/receive pipelines | User transport state |
| App state/UI | `IOS_APP/SmartCar_Control_MAC/.../Stores`, `UI`, `ViewModels` | `SmartCarViewModel` | telemetry/log stores, control views | Operator/developer UI |
