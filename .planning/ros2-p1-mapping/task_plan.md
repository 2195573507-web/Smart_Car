...
- 现有工作树有大量用户改动；本任务只新增本目录计划文件，不覆盖、不回退、不整理其它改动。

## 当前阶段

阶段 2b：第二次严谨性审查与开源复用核验（进行中；完成文档修订后等待用户批准）
...
| 2b. 第二次严谨性审查 | in_progress | 正在修正轮速 freshness、RF 极性、单 TCP owner、S3 依赖、时钟/序号和开源库版本 |
| 3. 只读遥测与里程计 bridge | pending | `/odom`、`odom -> base_link`、可选姿态话题 |
...
4. 轮速源提供可验证的 `sample_tick/sample_seq/valid` 或等价 stale 语义；旧版 `0x210` 只能作为实验数据，不能单独作为完成证据。
...
7. 人工驾驶低速绕行后 `/map` 可保存为 YAML/PGM，并可重新加载定位。
...
| ROS 能否看到姿态 | 当前 ROS2 没有姿态 topic。STM32 已产生 `ATTITUDE(0x201)` 和 `IMU_TELEMETRY(0x207)`，S3 目前主要转发给 App BLE；P1 可增加只读 telemetry 上行和 ROS topic，但姿态不是 2D 建图的硬依赖。`0x207` 的 LSM303 magnetometer 与 BMI323 gyro 必须分开映射。 |
...
- 轮序固定为 `[M1=RR, M2=RF, M3=LR, M4=LF]`。`(+1,-1,+1,+1)` 是 STM32 内部 raw encoder 校正，不是 `0x210` wire 极性；ROS2 默认倍率为 `[+1,+1,+1,+1]`，最终车辆方向仍必须实车验证。
- `0x210` 只有四个 float 且由 STM32 周期性重发缓存，未补充源采样时间/序号/valid 前不得把收包时间当作测量时间。
...
- SCBP inner sequence 是所有消息共享的 8 位序号；不能按 telemetry 类型直接计算连续性。
...
| 错误 | 次数 | 处理 |
| --- | --- | --- |
| 旧文档声称 `radar_uplink_protocol.c` 不存在 | 1 | 以当前源码为准；计划将文档漂移列为 P1 前置审计项，不回退源码。 |
