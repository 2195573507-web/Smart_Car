...
final class SmartCarViewModel: ObservableObject {
    ...
    func setJoystickInput(_ input: JoystickIntent) {
        ...
        cancelChassisCommand()
        if controlMode != .wheelIndependent {
            // A 0x15 four-wheel target is the established transition into
            // independent wheel control. Drop stale chassis targets first.
            bleManager.cancelQueuedMotionCommands()
            controlMode = .wheelIndependent
            chassisBaseSpeed = 0.0
            masterSpeedScale = 1.0
    ...
    func setJoystickSpeed(_ value: Double) {
        speed = max(0, min(100, value))
        if activeJoystickIntent != nil, status == .connected {
            applyJoystickIntent()
        }
    }
    ...
    private func acceptWheelControlTelemetry(_ status: WheelControlStatus) {
        // The joystick owns the commanded targets until it is released. The
        // telemetry store still records this status for feedback, but a stale
        // status frame must not replace a held operator command.
        guard Self.shouldApplyWheelControlTelemetry(
            activeJoystickIntent: activeJoystickIntent
        ) else {
            return
        }
    ...
    static func shouldApplyWheelControlTelemetry(
        activeJoystickIntent: JoystickIntent?
    ) -> Bool {
        activeJoystickIntent == nil
    }
    ...
    private func applyJoystickIntent() {
        ...
        wheelTargets = Self.wheelTargets(for: activeJoystickIntent, speed: joystickSpeed)
        markWheelControlPending()
        publishDriveIntent()
        scheduleWheelCommand()
    }
    ...
    private static func wheelTargets(for input: JoystickIntent, speed: Float) -> [Float] {
        ...
        let turnFraction = Float(pow(Double(abs(horizontal)), joystickSteeringCurve))
        ...
        // Preserve left/right steering semantics while reversing.
