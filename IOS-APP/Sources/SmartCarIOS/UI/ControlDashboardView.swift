    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 16) {
                ...
                DriveJoystickPanel(viewModel: viewModel)
                    .frame(maxWidth: .infinity)
                PosePanel(
...
private struct DriveJoystickPanel: View {
    @ObservedObject var viewModel: SmartCarViewModel
    @State private var translation = CGSize.zero
    @State private var lastInput: JoystickIntent?

    private var isEnabled: Bool { viewModel.status == .connected }

    var body: some View {
        Surface {
            VStack(spacing: 12) {
                SectionTitle(
                    title: "DRIVE JOYSTICK",
                    systemImage: "steeringwheel",
                    trailing: "(Int(viewModel.speed))%"
                )
                ...
                GeometryReader { proxy in
                    ...
                    ZStack {
                        ...
                        Circle()
                            ...
                            .offset(clampedTranslation(limit: limit))
                    }
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 0)
                            .onChanged { value in
                                ...
                                let input = joystickInput(for: value.translation, limit: limit)
                                if input != lastInput {
                                    lastInput = input
                ...
                Slider(
                    value: Binding(
                        get: { viewModel.speed },
                        set: { viewModel.setJoystickSpeed($0) }
                    ),
                    in: 0...100,
                    step: 1
                )
        ...
        .accessibilityLabel("行驶摇杆")
    }

    private func joystickInput(for translation: CGSize, limit: CGFloat) -> JoystickIntent {
        JoystickIntent(
    ...
    private func clampedTranslation(limit: CGFloat) -> CGSize {
        CGSize(
...
private struct JoystickCrosshair: View {
    var body: some View {
        GeometryReader { proxy in
            Path { path in
            ...
            .stroke(.secondary.opacity(0.3), style: StrokeStyle(lineWidth: 1, dash: [4, 5]))
        }
    }
}
