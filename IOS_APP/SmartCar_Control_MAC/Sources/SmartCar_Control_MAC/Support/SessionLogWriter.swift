...
final class SessionLogWriter: @unchecked Sendable {
    ...
    private lazy var timestampFormatter = Self.makeFormatter(
        "yyyy-MM-dd HH:mm:ss.SSS ZZZZZ"
    )

    static func makeDescriptor(connectedAt: Date) -> SessionLogDescriptor {
        let stem = "smartcar_log_\(format(connectedAt, as: "yyyy-MM-dd_HH-mm-ss"))"
        var fileName = "\(stem).md"
        var suffix = 1

        while FileManager.default.fileExists(
            atPath: logDirectoryURL.appendingPathComponent(fileName).path
        ) {
    ...
    func closeAndSynchronizeAndWait() {
        queue.sync {
            self.closeCurrentFile()
        }
    }
    ...
    private func openSession(_ descriptor: SessionLogDescriptor) {
        ...
        do {
            ...
            let header = "# SmartCar Session Log\n" +
            ...
            print("[SESSION_LOG] create failed: \(error.localizedDescription)")
        }
    }
    ...
    private func appendRecord(_ record: SmartCarLogRecord) {
        ...
        let line = "\(timestampFormatter.string(from: record.receivedAt)) " +
            "[\(record.source.displayName)][\(record.level.displayName)] " +
            "\(record.message)\n"
        do {
            try fileHandle.write(contentsOf: Data(line.utf8))
        } catch {
            print("[SESSION_LOG] append failed: \(error.localizedDescription)")
        }
    }
    ...
    private func closeCurrentFile() {
        ...
        do {
            ...
            print("[SESSION_LOG] synchronize failed: \(error.localizedDescription)")
        }

        do {
            try fileHandle.close()
        } catch {
            print("[SESSION_LOG] close failed: \(error.localizedDescription)")
        }
    }
