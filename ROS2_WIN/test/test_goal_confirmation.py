...
CORE = Path(__file__).parents[1] / "src" / "smartcar_bringup" / "scripts" / "goal_confirmation_core.py"
spec = importlib.util.spec_from_file_location("goal_confirmation_core", CORE)
...
def test_navigation_failure_clears_pending():
    state = module.GoalConfirmation()
    state.select(1.0, 2.0, 0.5)
    assert state.start(True)
