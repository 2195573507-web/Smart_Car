                          ...
                          size_t length)
{
    ...
    for (size_t index = 0U; index < length; ++index) {
        ...
        if (parser->length == 1U) {
            /* Version is fixed, so reject a false head before it can consume
             * a following valid frame while waiting for its declared length. */
                        ...
                        uint8_t *out, size_t capacity, uint16_t *out_length)
{
    ...
    if (length > SC_APP_FRAME_MAX_PAYLOAD ||
        capacity < SC_APP_FRAME_OVERHEAD + length) {
        return -2;
    }

    out[0] = SC_APP_FRAME_HEAD;
    out[1] = SC_APP_FRAME_VERSION;
    out[2] = type;
